#!/usr/bin/env python3
"""
QESCO Duplicate Bill Fetcher
Fetches electricity bill details from Quetta Electric Supply Company (QESCO) 
...
# Base PITC URL for QESCO duplicate bills
QESCO_BILL_URL = "https://bill.pitc.com.pk/qescobill"

def fetch_qesco_bill(identifier: str) -> dict:
    """
    Fetches the duplicate QESCO bill details using either:
    - 10-digit Customer ID / Consumer ID
    - 14-digit Reference Number
    """
    return fetch_bill_from_pitc(QESCO_BILL_URL, identifier, "QESCO")

def main():
    parser = argparse.ArgumentParser(
        description="Connects to QESCO and fetches details of the duplicate bill using Consumer ID or Reference Number.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python qesco_bill_fetcher.py --id 1234567890
  python qesco_bill_fetcher.py --id 01123456789012 --json
"""
    )
    parser.add_argument(
        "-i", "--id",
    ...
    try:
        bill_data = fetch_qesco_bill(args.id)
        
        if args.json:
            print(json.dumps(bill_data, indent=2))
        else:
            print("\n" + "="*50)
            print("         QESCO BILL DETAILS SUMMARY")
            print("="*50)
