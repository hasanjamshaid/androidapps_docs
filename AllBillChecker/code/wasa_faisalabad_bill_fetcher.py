#!/usr/bin/env python3
"""
WASA Faisalabad Duplicate Bill Fetcher
Fetches water and sewerage bill details from WASA Faisalabad using an 8-digit to 12-digit Account Number.
"""

import sys
import argparse
import json
from bill_utility import fetch_wasa_faisalabad_bill

def main():
    parser = argparse.ArgumentParser(
        description="Connects to WASA Faisalabad duplicate bill portal and fetches details using Account Number.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python wasa_faisalabad_bill_fetcher.py --id 12345678
  python wasa_faisalabad_bill_fetcher.py --id 12345678 --json
"""
    )
    parser.add_argument(
        "-i", "--id",
    ...
    try:
        bill_data = fetch_wasa_faisalabad_bill(args.id)
        
        if args.json:
            print(json.dumps(bill_data, indent=2))
        else:
            print("\n" + "="*50)
            print("       WASA FAISALABAD BILL DETAILS SUMMARY")
            print("="*50)
