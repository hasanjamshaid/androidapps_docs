#!/usr/bin/env python3
"""
WASA Rawalpindi Duplicate Bill Fetcher
Fetches water and sewerage bill details from WASA Rawalpindi using a 6-digit to 12-digit Consumer Number.
"""

import sys
import argparse
import json
from bill_utility import fetch_wasa_rawalpindi_bill

def main():
    parser = argparse.ArgumentParser(
        description="Connects to WASA Rawalpindi duplicate bill portal and fetches details using Consumer Number.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python wasa_rawalpindi_bill_fetcher.py --id 12345678
  python wasa_rawalpindi_bill_fetcher.py --id 12345678 --json
"""
    )
    parser.add_argument(
        "-i", "--id",
    ...
    try:
        bill_data = fetch_wasa_rawalpindi_bill(args.id)
        
        if args.json:
            print(json.dumps(bill_data, indent=2))
        else:
            print("\n" + "="*50)
            print("       WASA RAWALPINDI BILL DETAILS SUMMARY")
            print("="*50)
