#!/usr/bin/env python3
"""
GEPCO Duplicate Bill Fetcher
Fetches electricity bill details from Gujranwala Electric Power Company (GEPCO) 
...
# Base PITC URL for GEPCO duplicate bills
GEPCO_BILL_URL = "https://bill.pitc.com.pk/gepcobill"

def fetch_gepco_bill(identifier: str) -> dict:
    """
    Fetches the duplicate GEPCO bill details using either:
    - 10-digit Customer ID / Consumer ID
    - 14-digit Reference Number
    """
    return fetch_bill_from_pitc(GEPCO_BILL_URL, identifier, "GEPCO")

def main():
    parser = argparse.ArgumentParser(
        description="Connects to GEPCO and fetches details of the duplicate bill using Consumer ID or Reference Number.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python gepco_bill_fetcher.py --id 1234567890
  python gepco_bill_fetcher.py --id 01123456789012 --json
"""
    )
    parser.add_argument(
        "-i", "--id",
    ...
    try:
        bill_data = fetch_gepco_bill(args.id)
        
        if args.json:
            print(json.dumps(bill_data, indent=2))
        else:
            print("\n" + "="*50)
            print("         GEPCO BILL DETAILS SUMMARY")
            print("="*50)
